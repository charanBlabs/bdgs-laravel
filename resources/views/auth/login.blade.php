...
<form method="POST" action="{{ route('login.store') }}" class="bdgs-auth-form">
  @csrf

  <div class="bdgs-auth-field">
    <label for="email">Email Address<span class="bdgs-auth-required">*</span></label>
    <input type="email" id="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="email" @class(['bdgs-auth-input--error' => $errors->has('email')])>
  ...
  <div class="bdgs-auth-field">
    ...
    <div class="bdgs-auth-pw-wrap">
      <input type="password" id="password" name="password" required autocomplete="current-password" @class(['bdgs-auth-input--error' => $errors->has('password')])>
